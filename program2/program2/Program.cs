using System;

namespace program2
{
    class Program
    {
        private static void Wrapper()
        {

        }
        public static void Main(string[] args)
        {
            //string stringer = "Essa";
            //Console.WriteLine($"Ale mam dzisiaj fajnego stringa: {stringer}");
            //int num;

            //intro starwars start
            Console.Beep(294, 100);
            Console.Beep(294, 100);
            Console.Beep(294, 100);
            Console.Beep(391, 1000);
            Console.Beep(587, 1000);
            Console.Beep(523, 100);
            Console.Beep(493, 100);
            Console.Beep(440, 100);
            Console.Beep(783, 1000);
            Console.Beep(587, 500);
            Console.Beep(523, 100);
            Console.Beep(493, 100);
            Console.Beep(440, 100);
            Console.Beep(783, 1000);
            Console.Beep(587, 500);
            Console.Beep(523, 100);
            Console.Beep(493, 100);
            Console.Beep(523, 100);
            Console.Beep(440, 500);
            Console.Beep(37, 50);
            Console.Beep(294, 100);
            Console.Beep(294, 50);
            Console.Beep(391, 1000);
            Console.Beep(587, 1000);
            Console.Beep(523, 100);
            Console.Beep(493, 100);
            Console.Beep(440, 100);
            Console.Beep(783, 1000);
            Console.Beep(587, 500);
            Console.Beep(523, 100);
            Console.Beep(493, 100);
            Console.Beep(440, 100);
            Console.Beep(783, 1000);
            Console.Beep(587, 500);
            Console.Beep(523, 100);
            Console.Beep(493, 100);
            Console.Beep(523, 100);
            Console.Beep(440, 500);
            Console.Beep(37,50);
            Console.Beep(294, 100);
            Console.Beep(294, 50);
            Console.Beep(329, 750);
            Console.Beep(329, 250);
            Console.Beep(523, 100);
            Console.Beep(493, 100);
            Console.Beep(440, 100);
            Console.Beep(392, 100);
            Console.Beep(392, 100);
            Console.Beep(440, 100);
            Console.Beep(493, 100);
            Console.Beep(440, 100);
            Console.Beep(329, 250);
            Console.Beep(369, 500);
            Console.Beep(37, 50);
            Console.Beep(294, 100);
            Console.Beep(294, 50);
            Console.Beep(329, 750);
            Console.Beep(329, 250);
            Console.Beep(523, 100);
            Console.Beep(493, 100);
            Console.Beep(440, 100);
            Console.Beep(392, 100);
            Console.Beep(587, 500);
            Console.Beep(440, 250);
            Console.Beep(37, 50);
            Console.Beep(294, 100);
            Console.Beep(294, 50);
            //intro starwars end

            //string tmp = "";
            //int freq, duration;
            //while ((tmp = Console.ReadLine()).Length > 0)
            //{
            //    freq = Int32.Parse(tmp);
            //    duration = Int32.Parse(Console.ReadLine());
            //    Console.Beep(freq, duration);


            //    //bool isOk = Int32.TryParse(Console.ReadLine(), out num);
            //    //if (isOk)
            //    //{
            //    //    Console.WriteLine($"ok: {num}");
            //    //}
            //    //else
            //    //{
            //    //    Console.WriteLine("nie udało się");
            //    //}
            //}
        }
    }
}
